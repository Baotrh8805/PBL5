...
function showPostNotification(message, isError = false) {
    // Nếu là lỗi cấm (chứa chữ "hạn chế"), chúng ta dùng modal lớn
    if (isError && (message.includes("hạn chế đăng bài") || message.includes("hạn chế bình luận"))) {
        ...
        <div id="ban-alert-modal" style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.8); backdrop-filter: blur(8px); display: flex; justify-content: center; align-items: center; z-index: 10000; animation: modalFadeIn 0.3s ease-out;">
            <div style="background: #ffffff; border-radius: 20px; width: 480px; max-width: 92vw; overflow: hidden; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.5); position: relative; animation: modalSlideUp 0.4s cubic-bezier(0.175, 0.885, 0.32, 1.275);">
                ...
                <div style="padding: 45px 35px; text-align: center;">
                    ...
                    <h2 style="margin: 0 0 15px 0; font-size: 26px; color: #1c1e21; font-weight: 800; letter-spacing: -0.5px;">LC Networks cho biết</h2>
...
function openPostModal() {
    ...
    document.querySelector('.modal-header h2').innerText = 'Tạo bài viết';
    const btn = document.getElementById('modal-submit-btn');
    if(btn) btn.innerText = 'Đăng';
...
/**
 * Mở modal chỉnh sửa bài đăng
 */
async function openEditPostModal(postId) {
    editingPostId = postId;
    existingImageUrl = null;
    existingVideoUrl = null;
    document.querySelector('.modal-header h2').innerText = 'Chỉnh sửa bài viết';
    const btn = document.getElementById('modal-submit-btn');
    ...
    try {
        ...
        if (res.ok) {
            ...
            if (post.videoUrl) {
                existingVideoUrl = post.videoUrl;
            ...
            showPostNotification('Không thể tải bài viết để chỉnh sửa.', true);
        }
    } catch (err) {
        console.error(err);
        showPostNotification('Lỗi kết nối khi tải bài viết.', true);
    }
}
...
function checkModalPostContent() {
    ...
    const btn = document.getElementById('modal-submit-btn');
    if (text.length > 0 || selectedMediaFile || existingImageUrl || existingVideoUrl) {
        btn.disabled = false;
...
async function submitModalPost() {
    ...
    if (!content && !selectedMediaFile && !existingImageUrl && !existingVideoUrl) {
        return;
    }

    const btn = document.getElementById('modal-submit-btn');
    btn.innerText = editingPostId ? 'Đang lưu...' : 'Đang đăng...';
    btn.disabled = true;
    ...
    try {
        ...
        const url = isEditing ? `/api/posts/${editingPostId}` : '/api/posts/create';
        const method = isEditing ? 'PUT' : 'POST';

        // Gửi bài đăng đến endpoint
        ...
        if (res.ok) {
            ...
            if (isEditing) {
                ...
                showPostNotification('Đã lưu bài viết. Hệ thống sẽ kiểm duyệt lại trong nền.');
            } else {
                const inserted = typeof prependCreatedPostToFeed === 'function' && prependCreatedPostToFeed(returnedPost);
            ...
            if (res.status === 403) {
                ...
                showPostNotification(parseErrorMessage(errorData, isEditing ? 'Không thể lưu bài viết, vui lòng thử lại.' : 'Không thể đăng bài, vui lòng thử lại.'), true);
            }
        }
    } catch (error) {
        console.error(error);
        ...
        btn.innerText = editingPostId ? 'Lưu' : 'Đăng';
        checkModalPostContent();
    }
}
