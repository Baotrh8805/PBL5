...
async function loadDashboardData() {
    ...
    try {
        ...
        if (res.ok) {
            ...
            dashboardState.pending = cache.posts.length;
            syncDashboardStats();
        }
...
function renderPostsTable(posts, elementId) {
    ...
    tbody.innerHTML = posts.slice(0, 5).map(post => `
        <tr>
            <td>#P-${post.id}</td>
            <td><span class="status-badge warning">Cần duyệt</span></td>
            <td title="${escapeHtml(post.content || '')}">${post.content ? escapeHtml(post.content.substring(0, 30)) + '...' : '(Nội dung trống)'}</td>
            <td>${new Date(post.createdAt).toLocaleTimeString('vi-VN')}</td>
            <td>
                <button class="btn-action danger" onclick="deletePost(${post.id})">Xóa</button>
            </td>
        </tr>
    `).join('') || '<tr><td colspan="5">Trống</td></tr>';
}
...
function renderReviewPostsTable(posts) {
    ...
    const rows = posts.slice(0, 15).map(post => `
        <tr>
            ...
            <td title="${escapeHtml(post.content || '')}">${post.content ? escapeHtml(post.content.substring(0, 35)) + '...' : '(Nội dung trống)'}</td>
    ...
    tbody.innerHTML = rows || '<tr><td colspan="5" class="table-loading">Không có bài viết chờ duyệt.</td></tr>';
}
